...
const (
	...
	holder_regexp = `^([\p{L}'-]{3,})\s+[\p{L}' -]{3,}$`
)

func CheckCreditCard(ccn string) bool {
	match, err := regexp.MatchString(creditcard_regexp, ccn)
	if err != nil {
		return false
	}
	return match
...
func CheckHolderName(holder string) bool {
	match, err := regexp.MatchString(holder_regexp, holder)
	if err != nil {
		return false
	}
	return match
